...
interface TouchLayoutProfileDao {
    ...
    @Query("SELECT * FROM touch_layout_profiles ORDER BY platformFamilyId ASC")
    suspend fun listAll(): List<TouchLayoutProfileEntity>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsert(entity: TouchLayoutProfileEntity)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertAll(entities: List<TouchLayoutProfileEntity>)
    ...
    @Query("DELETE FROM touch_layout_profiles")
...
interface HardwareBindingProfileDao {
    ...
    @Query("SELECT * FROM hardware_binding_profiles ORDER BY platformFamilyId ASC")
    suspend fun listAll(): List<HardwareBindingProfileEntity>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsert(entity: HardwareBindingProfileEntity)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertAll(entities: List<HardwareBindingProfileEntity>)

    @Query("DELETE FROM hardware_binding_profiles")
    suspend fun deleteAll()
}
