...
import io.github.bandoracer.rommio.ui.component.QuickActionTile
...
fun HomeScreen(
    ...
    LazyColumn(
        ...
        item {
            val queuedCount = state.activeDownloads.count { it.status == io.github.bandoracer.rommio.model.DownloadStatus.QUEUED }
            val runningCount = state.activeDownloads.count { it.status == io.github.bandoracer.rommio.model.DownloadStatus.RUNNING }
            val failedCount = state.activeDownloads.count { it.status == io.github.bandoracer.rommio.model.DownloadStatus.FAILED }
            FeaturePanel(
                title = "Welcome back",
